...
function get_spell_name(school, index)
	...
	-- Hack -- 123 は OPT_english_others
	opt_english_others = op_ptr.opt[123]
	J_name = school.spells[index + 1].J_name
	if opt_english_others or not J_name then
		return school.spells[index + 1].name
	end
	return J_name
end
...
SPELL_MAGIC_MISSILE = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%dd4", 3 + ((player.lev - 1) / 5))
		end,
	effect = function()
...
SPELL_STINKING_CLOUD = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%d", 10 + (player.lev / 2))
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
SPELL_LIGHTNING_BOLT = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%dd6", (3 + ((player.lev - 5) / 6)))
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
{
	name = "Wonder",
	J_name = "謎", -- とりあえずWand of Wonderと同じ訳にしておく。
	effect = function()
...
SPELL_FROST_BOLT = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%dd8", (5 + ((player.lev - 5) / 4)))
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
SPELL_ACID_BOLT = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%dd8", (8 + ((player.lev - 5) / 4)))
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
SPELL_FIRE_BOLT = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%dd8", (6 + ((player.lev - 5) / 4)))
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
SPELL_HASTE_SELF = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 期間:%d+d20", player.lev)
		end,
	effect = function()
...
SPELL_TELEPORT_SELF = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 範囲:%d", player.lev * 5)
		end,
	effect = function()
...
SPELL_SHOCK_WAVE = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%d", 10 + player.lev)
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
SPELL_EXPLOSION = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%d", 20 + player.lev * 2)
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
SPELL_CLOUD_KILL = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%d", 40 + (player.lev / 2))
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
SPELL_REND_SOUL = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:11d%d", player.lev)
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
{
	name = "Chaos Strike",
	J_name = "カオス・アタック", -- mada
	info = function()
			--return string.format(" dam 13d%d", player.lev)
			return string.format(" 損傷:13d%d", player.lev)
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
SPELL_FROST_BALL = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%d", 30 + player.lev)
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
SPELL_ACID_BALL = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%d", 40 + player.lev)
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
SPELL_FIRE_BALL = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%d", 55 + player.lev)
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
SPELL_ICE_STORM = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%d", 50 + (2 * player.lev))
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
SPELL_METEOR_SWARM = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%dx%d", 30 + player.lev / 2,
			              2 + player.lev / 20)
		end,
	effect = function()
			local success, dir = get_aim_dir()
...
SPELL_RIFT = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:40+%dd7", player.lev)
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
SPELL_MANA_STORM = add_magic_spell
{
	...
	info = function()
			...
			return string.format(" 損傷:%d", 300 + (player.lev * 2))
		end,
	effect = function()
			local success, dir = get_aim_dir()
			if not success then return false end
...
PRAYER_PORTAL = add_prayer
{
	...
	info = function()
			...
			return string.format(" 範囲:%d", 3 * player.lev)
		end,
	effect = function()
...
PRAYER_PROTECTION_FROM_EVIL = add_prayer
{
	...
	info = function()
			...
			return string.format(" 期間:%d+d25", 3 * player.lev)
		end,
	effect = function()
...
PRAYER_DISPEL_UNDEAD = add_prayer
{
	...
	info = function()
			...
			return string.format(" 損傷:d%d", 3 * player.lev)
		end,
	effect = function()
...
PRAYER_DISPEL_UNDEAD2 = add_prayer
{
	...
	info = function()
			...
			return string.format(" 損傷:d%d", 4 * player.lev)
		end,
	effect = function()
...
PRAYER_TELEPORT_SELF = add_prayer
{
	...
	info = function()
			...
			return string.format(" 範囲:%d", 8 * player.lev)
		end,
	effect = function()
